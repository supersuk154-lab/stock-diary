...
def _show_pin_entry(
    ...
    if locked_until_str:
        try:
            ...
            if now < locked_until:
                ...
                st.markdown("---")
                if st.button("이메일로 로그인", width='stretch'):
    ...
    with st.form("pin_entry_form"):
        ...
        submit = st.form_submit_button("확인", type="primary", width='stretch')
    ...
    with col1:
        if st.button("이메일로 로그인", width='stretch'):
            ...
            st.rerun()
    with col2:
        if st.button("PIN 초기화", width='stretch'):
            clear_pin_cache()
            st.session_state["_show_full_login"] = True
            st.rerun()
...
def _show_pin_setup(dev_mode: bool):
    ...
    with st.form("pin_setup_form"):
        ...
        col1, col2 = st.columns(2)
        with col1:
            save_btn = st.form_submit_button("PIN 설정", type="primary", width='stretch')
        with col2:
            skip_btn = st.form_submit_button("건너뛰기", width='stretch')
