...
def _render_step_final(supabase, ai_client, selected_tags):
...
* 주의사항 3: extracted_trades의 각 항목에는 stock_name, quantity, type("buy" 또는 "sell")을 모두 포함해야 합니다.
"""
            ...
            if err:
                ...
                try:
                    ...
                    try:
                        # 1. 일기 저장
                        resp = supabase.table("journals").insert({
                            "user_id":     _uid,
                            "tags":        tags_str,
...
def render_upload_section(supabase, ai_client, selected_tags):
    ...
    if step == 'upload_mode':
        _render_step_upload(supabase, ai_client)
    elif step == 'verify_data':
        _render_step_verify(supabase)
    elif step == 'ask_next':
        _render_step_ask_next()
    elif step == 'final_analysis':
        _render_step_final(supabase, ai_client, selected_tags)
