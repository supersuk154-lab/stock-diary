...
def safe_generate(client, model_name, contents, config=None, fallback_msg="AI 분석 중 오류가 발생했어요."):
    """새로운 google-genai SDK 규격에 맞춘 안전망 함수 + 429 한도 초과 시 gemini-2.5-flash 자동 폴백"""
    try:
        ...
        # 429 또는 RESOURCE_EXHAUSTED 한도 도달 시 gemini-2.5-flash로 자동 폴백 시도
        is_429 = "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e) or "quota" in str(e).lower()
        if is_429 and model_name != "gemini-2.5-flash":
            logger.info("Attempting automatic fallback to gemini-2.5-flash due to quota/429 error...")
            try:
                response = client.models.generate_content(
                ...
                logger.exception("Fallback to gemini-2.5-flash also failed")
